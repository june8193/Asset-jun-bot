...
### 0단계: 주말 및 휴장일 여부 확인 (Pre-check)
- [ ] **휴장일 판정 도구 실행**: 쉘 명령어 실행 도구를 통해 `uv run python scripts/query_market.py --action holiday --country US` 명령을 실행하여 미국 시장 휴장일 정보(`DATE`, `COUNTRY`, `IS_HOLIDAY`, `DESCRIPTION`)를 확인합니다.
...
### 1단계: 지수 데이터 및 뉴스 수집
- [ ] **지수 데이터 조회 선택**:
  - `IS_HOLIDAY`가 `False`인 경우(정상 영업일): 쉘 명령어 실행 도구를 통해 `uv run python scripts/query_market.py --action indices --country US` 명령을 실행하여 오늘 마감된 미국 3대 지수(S&P 500, NASDAQ, DOW JONES) 데이터를 획득합니다.
...
- [ ] **마크다운 파일 생성 및 저장**: 파일 쓰기 도구(`write_to_file` 등)를 호출하여 `STORAGE_DIR/reports/us_market/daily/US_market_daily_report_YYYYMMDD.md` (YYYYMMDD는 오늘 날짜) 경로에 아래 템플릿 규격에 맞춘 새로운 마크다운 파일을 직접 생성하여 저장합니다.
  ...
  - **보고서 템플릿**:
    ...
    - **휴장일 (is_holiday=true)인 경우**:
      ...
      - **분석 시간 (미국)**: YYYY-MM-DD (요일)
      ---
      금일은 미국 주식 시장 휴장일(사유: [description])입니다.
      ```
...
- [ ] **메시지 구성**:
  ...
  - **휴장일 (is_holiday=true)인 경우**: 단문 메시지로 텔레그램 메시지를 구성합니다.
    ```markdown
    금일은 미국 주식 시장 휴장일([description])로 인해 데일리 분석이 제공되지 않습니다.
    ```
...
- [ ] **메시지 전송**: 쉘 명령어 실행 도구(`run_command` 등)를 호출하여 텔레그램 메시지 전송 스크립트를 구동합니다.
  - 실행할 명령어:
    ...
    ```
- [ ] **완료 검증**: 전송이 성공적으로 끝났는지 로그 출력("Telegram message sent successfully...")을 통해 검증합니다. 휴장일 경우 텔레그램 전송이 완료되면 작업을 즉시 완전히 종료합니다.
