...
description: Generate, save, and convert the weekly KOSPI/KOSDAQ index status report into Markdown and PDF formats by reading daily reports of the past week, query weekly index change, and send notification. Use when executing weekly market index briefings.
---

# 국내 시장 주간 지수 현황 보고서 작성 (Korea Weekly Index Report)

이 스킬은 예약된 시간에 에이전트가 직접 도구(Tools)를 체인으로 호출하여 가장 최근에 완료된 한 주(일요일~토요일)의 국내 시장(KOSPI, KOSDAQ) 일일 보고서들을 수집·요약하고, 주간 지수 변동률 데이터를 획득한 뒤 종합 주간 마크다운 보고서를 작성한 뒤, PDF로 변환하여 지정된 저장 공간에 보관하고 텔레그램으로 전송합니다.
...
- [ ] **마크다운 파일 생성**: 파일 쓰기 도구(`write_to_file` 등)를 호출하여 `STORAGE_DIR/reports/korea_market/weekly/Korea_market_weekly_report_YYYYMMDD.md` (YYYYMMDD는 요약 주의 **토요일** 날짜) 경로에 아래 템플릿 규격에 맞춰 새로운 마크다운 파일을 생성 및 저장합니다.
  - **주간 보고서 템플릿**:
    ...
    ### 📊 주간 지수 변동 현황
    - KOSPI: [시작가격] -> [종료가격] 포인트 ([변동률]%)
    - KOSDAQ: [시작가격] -> [종료가격] 포인트 ([변동률]%)
