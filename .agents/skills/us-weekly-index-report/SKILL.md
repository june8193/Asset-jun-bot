...
- [ ] **일일 보고서 파일 조회**: 1단계에서 산출된 7일간의 날짜(일요일 ~ 토요일)에 대해 각 날짜별 일일 마크다운 보고서 경로를 구성하여 존재 여부를 확인합니다.
  - 미국 일일 보고서 경로: `STORAGE_DIR/reports/us_market/daily/US_market_daily_report_YYYYMMDD.md`
...
### 3단계: 주간 지수 변동 데이터 조회
- [ ] **주간 지수 변동 분석 획득**: MCP 도구 호출 기능(`call_mcp_tool`)을 통해 `ServerName="assetmanager"`, `ToolName="get_market_history"`, `Arguments={"tickers": "^GSPC,^IXIC,^DJI", "start_date": "[1단계의 시작일 YYYY-MM-DD]", "end_date": "[1단계의 종료일 YYYY-MM-DD]"}` 형식으로 호출하여 지정된 주간 범위의 역사적 지수 데이터를 조회합니다.
...
- [ ] **마크다운 파일 생성**: 파일 쓰기 도구(`write_to_file` 등)를 호출하여 `STORAGE_DIR/reports/us_market/weekly/US_market_weekly_report_YYYYMMDD.md` (YYYYMMDD는 요약 주의 **토요일** 날짜) 경로에 아래 템플릿 규격에 맞춰 새로운 마크다운 파일을 생성 및 저장합니다.
  - **주간 보고서 템플릿**:
    ```markdown
    # 📢 미국 주식 시장 주간 현황 보고서
    ...
    ### 💡 주간 종합 분석
    [각 일일 보고서의 종합 분석 및 핵심 이벤트를 유기적으로 결합하고, 한 주의 주요 흐름(주요 뉴스 이슈 포함)과 향후 전망을 구체적으로 요약하여 기술 (뉴스 섹션이 제거되었으므로 종합 분석에 주요 사건들이 빠짐없이 반영되도록 함)]
    ```
  - ⚠️ 주의: 텔레그램 등 메신저 연동 시 깨짐 방지를 위해 절대 **표(Table) 서식은 사용하지 마십시오**.

### 5단계: PDF 변환 실행
- [ ] **변환 명령어 실행**: 쉘 명령어 실행 도구(`run_command` 등)를 통해 PDF 변환 스크립트를 구동합니다.
  - 마크다운 경로: `STORAGE_DIR/reports/us_market/weekly/US_market_weekly_report_YYYYMMDD.md`
  - PDF 저장 경로: `STORAGE_DIR/reports/us_market/weekly/US_market_weekly_report_YYYYMMDD.pdf`
  - 실행 명령어:
    ```bash
