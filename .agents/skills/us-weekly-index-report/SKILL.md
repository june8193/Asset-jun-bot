...
### 3단계: 주간 지수 변동 데이터 조회
- [ ] **주간 지수 변동 분석 획득**: `assetmanager` MCP 도구의 `get_market_history(tickers="^GSPC,^IXIC,^DJI", start_date="[1단계의 시작일 YYYY-MM-DD]", end_date="[1단계의 종료일 YYYY-MM-DD]")`를 호출하여 지정된 주간 범위의 역사적 지수 데이터를 조회합니다.
