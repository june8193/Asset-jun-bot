...
준과 성은 부부의 통합 자산 정보를 조회하고 장 마감 거래내역 자동 동기화 및 보고서를 전송하는 텔레그램 봇 프로젝트입니다.  
텔레그램 CLI 명령어(/asset, /ratio, /tx, /sync 등) 및 스케줄러 기반 자동 알림을 제공하며, 심층 AI 분석 및 대화는 Antigravity Remote Control 및 에이전트 스킬을 통해 수행합니다.
...
2. **환경 변수 설정**
   ...
   STORAGE_DIR="your_storage_directory_path"
   ASSET_MANAGER_API_URL="http://localhost:8000"
   ASSET_MANAGER_DIR="your_assetmanager_directory_path"
...
### 1. 시스템 아키텍처
Asset-jun-bot은 다음과 같은 흐름으로 사용자의 CLI 요청을 처리하고 장 마감 자동 동기화를 수행합니다.

```mermaid
graph TD
    User(["사용자 (Telegram)"]) <--> TelegramBot["telegram_bot/bot.py"]
    TelegramBot <--> CLICommandHandler["telegram_bot/commands.py"]
    TelegramBot <--> TelegramScheduler["telegram_bot/scheduler.py"]
    CLICommandHandler <--> AssetClient["asset_client/"]
    TelegramScheduler <--> AssetClient
...
* **[src/asset_jun_bot/](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot)**
  ...
  * [telegram_bot/](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/telegram_bot): 텔레그램 봇 모듈 (롱 폴링, CLI 커맨드 핸들러, 스케줄러, 렌더러).
  * [asset_client/](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/asset_client): `AssetManager API`와 연동하여 실시간 자산 데이터를 가져오는 HTTP 클라이언트 패키지입니다.
...
* **[.agents/skills/](file:///c:/localrepo/Asset-jun-bot/.agents/skills)**
  * Antigravity Remote Control 및 에이전트 작업에 활용되는 자산 진단 및 마켓 리포트 스킬 모음입니다.
...
| :--- | :--- |
| **텔레그램 메시지 송수신 로직 및 사용자 차단/인증** | [telegram_bot/bot.py](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/telegram_bot/bot.py) |
| **텔레그램 CLI 커맨드 핸들러 및 응답 포맷팅** | [telegram_bot/commands.py](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/telegram_bot/commands.py) |
| **장 마감 자동 동기화 스케줄러** | [telegram_bot/scheduler.py](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/telegram_bot/scheduler.py) |
| **AssetManager API 연동 로직 및 호출 엔드포인트 수정** | [asset_client/](file:///c:/localrepo/Asset-jun-bot/src/asset_jun_bot/asset_client) |
